(ns cdc.main
  "Kotodama WASM entrypoint for the CDC clean-room actor (L4) — Clojure port.
  ...
  facts (`cdc.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"cdc.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cdc-compat" :tier tier :entities entities} 200])
